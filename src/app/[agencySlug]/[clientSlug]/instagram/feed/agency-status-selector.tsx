...
export function AgencyPubStatusSelector({ id, status, onPost }: Props) {
  ...
  return (
    <Menubar className="p-0 m-0 bg-transparent border-none">
      <MenubarMenu>
        ...
        <MenubarContent>
          <MenubarItem onClick={() => handleClick(PublicationStatus.BORRADOR)}>
            {getNode(PublicationStatus.BORRADOR)}
          </MenubarItem>
          <MenubarItem onClick={() => handleClick(PublicationStatus.REVISADO)}>
            {getNode(PublicationStatus.REVISADO)}
          </MenubarItem>
          <MenubarItem onClick={() => handleClick(PublicationStatus.APROBADO)}>
            {getNode(PublicationStatus.APROBADO)}
          </MenubarItem>
          <MenubarItem onClick={() => handleClick(PublicationStatus.PROGRAMADO)}>
            {getNode(PublicationStatus.PROGRAMADO)}
          </MenubarItem>
          <MenubarItem onClick={() => handleClick(PublicationStatus.PUBLICADO)}>
            {getNode(PublicationStatus.PUBLICADO)}
          </MenubarItem>
        </MenubarContent>
      </MenubarMenu>
    </Menubar>
