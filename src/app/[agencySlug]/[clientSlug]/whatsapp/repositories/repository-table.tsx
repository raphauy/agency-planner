...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex gap-1 dark:text-white items-center">
          ...
          <Input className="max-w-xs" placeholder="functionDescription filter..."
              value={(table.getColumn("functionDescription")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("functionDescription")?.setFilterValue(event.target.value)}                
          />
          
      
          <Input className="max-w-xs" placeholder="executionResponse filter..."
              value={(table.getColumn("executionResponse")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("executionResponse")?.setFilterValue(event.target.value)}                
          />
