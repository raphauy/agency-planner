...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex gap-1 dark:text-white items-center">
          ...
          <Input className="max-w-xs" placeholder="creditFactor filter..."
              value={(table.getColumn("creditFactor")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("creditFactor")?.setFilterValue(event.target.value)}                
          />
